...
def configure_torch_threads():
    thread_env = os.environ.get("TORCH_NUM_THREADS") or os.environ.get("OMP_NUM_THREADS")
    interop_env = os.environ.get("TORCH_NUM_INTEROP_THREADS", "1")
    if thread_env:
        torch.set_num_threads(max(1, int(thread_env)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--m4_result_dir', type=str, default='',
                        help='subdirectory under m4_results for M4 forecasts')
    parser.add_argument('--inverse', action='store_true', help='inverse output data', default=False)
    parser.add_argument('--reversal_case', type=str, default='NN', choices=['NN', 'RN', 'NR', 'RR'],
                        help='data reversal case: first letter controls train, second controls val/test')
    parser.add_argument('--reversal_mask_path', type=str, default='',
                        help='JSON mask selecting the channels/series eligible for reversal')
    parser.add_argument('--channel_results_root', type=str, default='',
                        help='root directory for per-channel/per-series evaluation files')
    parser.add_argument('--skip_visualization', action='store_true', default=False,
                        help='skip forecast PDF generation during large experiment grids')
    ...
    parser.add_argument('--save_raw_predictions', action='store_true', default=False,
                        help='save pred.npy and true.npy arrays in results (large; default: off)')
    ...
    args.reversal_case = args.reversal_case.upper()
    # Honor the command-line seed.  Earlier experiment bundles were generated
    # with a hard-coded seed of 2021; keeping the parsed --seed unused made a
    # requested multi-seed replication silently repeat the same RNG stream.
