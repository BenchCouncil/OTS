...
try:
    ...
    subsample = interpolate_missing = None
    Normalizer = None
import warnings
...
def _normalize_reversal_case(args):
    case = getattr(args, 'reversal_case', 'NN').upper()
    if case not in {'NN', 'RN', 'NR', 'RR'}:
        raise ValueError("reversal_case must be one of NN, RN, NR, or RR")
    return case


def _should_reverse_split(args, flag):
    case = _normalize_reversal_case(args)
    if flag == 'train':
        return case[0] == 'R'
    if flag in {'val', 'test'}:
        return case[1] == 'R'
...
def _selection_mask(args, identifiers):
    """Return the fixed partial-reversal mask for channel names or M4 ids."""
    identifiers = [str(identifier) for identifier in identifiers]
    mask_path = getattr(args, 'reversal_mask_path', '')
    if not mask_path:
        return np.ones(len(identifiers), dtype=bool)
    with open(mask_path, 'r', encoding='utf-8') as handle:
        manifest = json.load(handle)
    selected = {str(identifier) for identifier in manifest.get('selected_ids', [])}
    if not selected and 'selected_indices' in manifest:
        selected_indices = {int(index) for index in manifest['selected_indices']}
        return np.array([index in selected_indices for index in range(len(identifiers))], dtype=bool)
    return np.array([identifier in selected for identifier in identifiers], dtype=bool)


def _reverse_split_if_needed(args, flag, data_x, data_y, data_stamp, selected_mask=None):
    if not _should_reverse_split(args, flag):
        return data_x, data_y, data_stamp
    if getattr(args, 'reversal_mask_path', ''):
        if selected_mask is None:
            raise ValueError('A partial reversal mask was requested but no channel mask was supplied')
        selected_mask = np.asarray(selected_mask, dtype=bool)
        if selected_mask.shape != (data_x.shape[-1],):
            raise ValueError(f'Reversal mask shape {selected_mask.shape} does not match {data_x.shape[-1]} channels')
        data_x = data_x.copy()
        data_y = data_y.copy()
        data_x[:, selected_mask] = data_x[::-1, selected_mask]
        data_y[:, selected_mask] = data_y[::-1, selected_mask]
        # Mixed-direction channels cannot share a reversed calendar embedding. Keeping
        # stamps fixed also guarantees that unselected channels are truly unchanged.
        return data_x, data_y, data_stamp
    return data_x[::-1].copy(), data_y[::-1].copy(), data_stamp[::-1].copy()
...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        self.channel_names = [str(column) for column in df_data.columns]
        self.reversal_selected_mask = _selection_mask(self.args, self.channel_names)
        ...
        self.data_x, self.data_y, data_stamp = _reverse_split_if_needed(
            self.args, self.flag, self.data_x, self.data_y, data_stamp, self.reversal_selected_mask)
...
class Dataset_M4(Dataset):
    ...
    def __read_data__(self):
        ...
        training_values = np.array(
            ...
            dtype=object)  # split different frequencies
        self.ids = np.array([i for i in dataset.ids[dataset.groups == self.seasonal_patterns]])
        self.timeseries = [ts for ts in training_values]
        self.reversal_selected_mask = _selection_mask(self.args, self.ids)
        if _should_reverse_split(self.args, self.flag):
            self.timeseries = [
                ts[::-1].copy() if self.reversal_selected_mask[index] else ts
