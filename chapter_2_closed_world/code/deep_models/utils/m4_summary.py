...
def _clean_series(values):
    ...
    for v in values:
        ...
        cleaned.append(v[~np.isnan(v)])
    return cleaned


def group_values(values, groups, group_name):
    cleaned = _clean_series(values[groups == group_name])
...
class M4Summary:
    ...
    def evaluate(self):
        ...
        naive2_forecasts = np.array(_clean_series(naive2_forecasts), dtype=object)
        ...
        for group_name in M4Meta.seasonal_patterns:
            ...
            if os.path.exists(file_name):
                forecast_frame = pd.read_csv(file_name)
                if 'id' in forecast_frame.columns:
                    forecast_frame = forecast_frame.drop(columns=['id'])
                model_forecast = forecast_frame.select_dtypes(include=[np.number]).values
